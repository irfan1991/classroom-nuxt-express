      ...
      <div class="col-md-4" v-for="(item,i) in listClass" :key="i">
        <class-item-2 :title="item.name" :id="item.id" :description="item.description" :img="item.photo">
          <template v-slot:footer>
            <p @click="entryClass(item)">Diikuti sebanyak : {{item.students? item.students.length : 0}} Orang</p>
            <p>Pemateri : {{item.tutors ? item.tutors[0].name : 'Belum ada pengajar' }}</p>
            <center>
                <button @click="entryClass(item)" class="btn btn-lg btn-danger" type="button">Join Kelas</button>
            </center>
          </template>
        </class-item-2>
      </div>
